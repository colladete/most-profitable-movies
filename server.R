...
server <- function(input, output) {
  ...
  films_sel <-  reactive({ movie_profit_data[grep(tolower(input$movname), tolower(movie_profit_data$movie_title)),] %>%
      mutate('Movie title' = movie_title, 'Profit' = profit, 'Budget in $' = formatC(budget, format="d", big.mark=","), 'Gross in $' = formatC(gross, format="d", big.mark=",")) %>%
      select(`Movie title`, Profit, `Budget in $`, `Gross in $`)
  ...
  most.prof <- reactive({
    ...
    movie_profit_data %>%
      ...
      mutate(Genre = ifelse(is.na(genre2), genre1, paste(genre1, genre2, sep = ",")),
             'Movie title' = movie_title, Profit = profit, Year = year, 'Budget in $' = formatC(budget, format="d", big.mark=","),
             'Gross in $' = formatC(gross, format="d", big.mark=",")) %>%
